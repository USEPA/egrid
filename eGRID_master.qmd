...
Three data tables are downloaded from the the CAMD api and stored into one .RDS file, `camd_raw.RDS`. This file consists of facility attributes, annual emissions data, and containing annual emissions data for the ozone months. See a summary of the raw CAMD file below, separated into categories.
...
camd_facilities      <- camd_raw[1:26]
camd_emissions       <- camd_raw[27:46]
camd_emissions_ozone <- camd_raw[47:58]
...
**CAMD facility attributes**: Columns 1 - 26 of `camd_raw.RDS`.

**CAMD annual emissions**: Columns 27 - 47 of `camd_raw.RDS`.

**CAMD ozone season emissions**: Columns 47 - 58 of `camd_raw.RDS`. The ozone season is defined as May through September.
...
Crosswalks and static tables are used to supplement CAMD and EIA files and provide information on one-off changes, descriptions, or overall conversions.

-   Units to add, delete, or account for differences between CAMD and EIA

    -   ex. `units_to_remove.csv`, `xwalk_oris_camd.csv`

-   Data on information not found in EIA/CAMD
...
The generator file extracts data from `eia_860_clean.RDS`, including all operable and retired generators . The code pulls variables from EIA-860 data (`eia_860$boiler_generator` and `eia_860$combined`) and counts the number of boilers. Then, we assign generation to each generator value.

Direct generation is assigned to each generator values through the `EIA-923 Generator` file. For values not included in the `EIA-923 Generator` file, generation is determined by using a nameplate capacity ratio with `EIA-923 Generator and Fuel` data. A generator level capacity factor is calculated, and a final round of updates for fuel types, plant codes, and plant names is applied.
...
The unit file data includes grid connected units from EPA/CAMD data, unique EIA-923 boilers, unique EIA-860 boilers, and renewable units from EIA not included previously. Some units are ozone season reporters where they only reports data during the ozone season of May - September. After flagging which CAMD plants are ozone season reporters, we fill in the missing months with EIA data.

Unit level emissions come from CAMD Power Sector Emissions Data. For units not included in CAMD, but in EIA, the emissions are calculated through reported unit level heat input from EIA.
...
plot_unt_hg <-
  unit_file %>%
  ggplot(aes(x = hg_mass)) +
  geom_histogram() +
  theme_minimal() +
  labs(title = "Hg Mass")

plot_unt_hg_sources_dist <-
  unit_file %>%
  count(hg_source) %>%
  mutate(proportion = n/sum(n)) %>%
  ggplot(aes(x = hg_source, y = proportion)) +
  geom_col() +
  coord_flip() +
  scale_x_discrete(labels = scales::wrap_format(30)) +
  scale_y_continuous(labels = scales::label_percent()) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of Hg Mass Data Sources",
    y = "Share of units",
    x = NULL
    )


plot_unt_hg / plot_unt_hg_sources_dist
...
```{r}
#| label: plnt_file_heat_in_oz_dist
#| warning: false
#| message: false
#| fig-height: 10
...
plot_plnt_unadj_hg <-
  plant_file %>%
  ggplot(aes(x = unadj_hg_mass)) +
  geom_histogram() +
  theme_minimal() +
  labs(title = "Unadjusted Hg Mass")

plot_plnt_hg_rate <-
  plant_file %>%
  ggplot(aes(x = hg_out_emission_rate)) +
  geom_histogram() +
  theme_minimal() +
  #xlim(c(0, 100)) + # adding in limit to deal with outliers
  labs(title = "Hg Emission Output Rate")

plot_plnt_hg_dist <-
  plant_file %>%
  count(unadj_hg_source) %>%
  mutate(proportion = n/sum(n)) %>%
  ggplot(aes(x = unadj_hg_source, y = proportion)) +
  geom_col() +
  coord_flip() +
  scale_x_discrete(labels = scales::wrap_format(30)) +
  scale_y_continuous(labels = scales::label_percent()) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of Unadj. Hg Mass Data Sources",
    y = "Share of units",
    x = NULL
    )


(plot_plnt_hg + plot_plnt_unadj_hg) / plot_plnt_hg_rate / plot_plnt_hg_dist
...
The demographic file uses geographic coordinates from all plants and EPA's EJScreen tool to calculate the demographic characteristics of surrounding neighborhoods. For more information on the analysis process, [view EJScreen's peer-reviewed and tested methodology](https://www.epa.gov/ejscreen/technical-information-and-data-downloads).
