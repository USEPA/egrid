...
params:
  eGRID_year: "2024"
  version: "1.0.2"
  temporal_res: "monthly" # temporal resolution of output either: 
                         # "annual" or "monthly"
                         # NOTE: annual MUST be run before monthly to 
                         #       obtain all necessary data for formatting monthly
editor: visual
project: 
  execute_dir: project
---
...
```{r}
#| label: load-names
#| file: "scripts/1_production_model/name_matching.R"
#| echo: false
...
path <- glue::glue("data/1_production_model/static_tables")
...
eia_860_files <- read_rds(
        glue::glue("data/1_production_model/clean_data/eia/{params$eGRID_year}/eia_860_clean.RDS"))
eia_861_files <- read_rds(
        glue::glue("data/1_production_model/clean_data/eia/{params$eGRID_year}/eia_861_clean.RDS"))
```
...
#| file: "scripts/1_production_model/generator_file_create.R"
#| results: hold
```
...
generator_file <- readr::read_rds(
        glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/generator_file_{params$temporal_res}.RDS"))
...
create_summary_gt_table(generator_file)
```
...
plot_gen_source <-
  generator_file %>%
  ggplot(aes(x = gen_data_source, y = generation)) +
  geom_boxplot() +
  theme_minimal() +
  coord_flip() +
  scale_x_discrete(labels = scales::wrap_format(30)) +
  labs(title = "Total Generation by Data Source")

if (params$temporal_res == "annual") {
  (plot_gen + plot_gen_oz) / plot_gen_source
} else {
plot_gen / plot_gen_source
}
...
#| file: "scripts/1_production_model/unit_file_create.R"
#| results: hold
```
...
unit_file <- readr::read_rds(
      glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/unit_file_{params$temporal_res}.RDS"))
```
...
create_summary_gt_table(unit_file)
```
...
plot_unt_heat_in / plot_unt_heat_in_source_dist
}
...
plot_unt_nox / plot_unt_nox_sources_dist
}
...
#| file: "scripts/1_production_model/plant_file_create.R"
#| results: hold
```
...
plant_file <- readr::read_rds(
        glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/plant_file_{params$temporal_res}.RDS"))
```
...
create_summary_gt_table(plant_file)
```
...
plot_plnt_nox_rate <-
  plant_file %>%
  ggplot(aes(x = nox_output_rate)) +
  geom_histogram() +
  theme_minimal() +
  xlim(c(0, 150)) + # adding in limit to deal with outliers
  labs(title = "NOx Output Emission Rate")
...
plot_plnt_nox_oz_rate <-
  plant_file %>%
  ggplot(aes(x = nox_oz_output_rate)) +
  geom_histogram() +
  theme_minimal() +
  xlim(c(0, 150)) + # adding in limit to deal with outliers
  labs(title = "NOx Ozone Output Emission Rate")
...
plot_plnt_so2_rate <-
  plant_file %>%
  ggplot(aes(x = so2_output_rate)) +
  geom_histogram() +
  theme_minimal() +
  xlim(c(0, 50)) + # adding in limit to deal with outliers
  labs(title = "SO2 Output Emission Rate")
...
plot_plnt_co2_rate <-
  plant_file %>%
  ggplot(aes(x = co2_output_rate)) +
  geom_histogram() +
  theme_minimal() +
  xlim(c(0, 30000)) + # adding in limit to deal with outliers
  labs(title = "CO2 Output Emission Rate")
...
plot_plnt_ch4_rate <-
  plant_file %>%
  ggplot(aes(x = ch4_output_rate)) +
  geom_histogram() +
  theme_minimal() +
  xlim(c(0, 10)) + # adding in limit to deal with outliers
  labs(title = "CH4 Output Emission Rate")
...
plot_plnt_n2o_rate <-
  plant_file %>%
  ggplot(aes(x = n2o_output_rate)) +
  geom_histogram() +
  theme_minimal() +
  xlim(c(0, 1)) + # adding in limit to deal with outliers
  labs(title = "N2O Output Emission Rate")
...
#| file: "scripts/1_production_model/region_aggregation_create.R"
#| results: hold
```
...
ba_agg  <- read_rds(
    glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/ba_aggregation_{params$temporal_res}.RDS"))

st_agg  <- read_rds(
    glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/state_aggregation_{params$temporal_res}.RDS"))

srl_agg <- read_rds(
    glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/subregion_aggregation_{params$temporal_res}.RDS"))

nrl_agg <- read_rds(
    glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/nerc_aggregation_{params$temporal_res}.RDS"))

us_agg  <- read_rds(
    glue::glue("data/1_production_model/outputs/{params$eGRID_year}/{params$temporal_res}/us_aggregation_{params$temporal_res}.RDS"))
```
...
create_summary_gt_table(ba_agg)
```
...
create_summary_gt_table(st_agg)
```
...
create_summary_gt_table(srl_agg)
```
...
create_summary_gt_table(nrl_agg)
```
...
create_summary_gt_table(us_agg)
```
:::
...
#| file: "scripts/1_production_model/grid_gross_loss_create.R"
#| results: hold
#| error: true
```
...
if(ggl_file_exists){
  ggl_file <- readr::read_rds(
             glue::glue("data/1_production_model/outputs/{params$eGRID_year}/grid_gross_loss.RDS"))
}
```
...
if(ggl_file_exists){
  create_summary_gt_table(ggl_file)
}
...
The metric file script produces a metric version of the eGRID datasets by converting certain data from imperial to metric units. The output file follows this format: `{file_name}_{params$temporal_res}_metric.RDS` The results are the following metric version files:
...
conversion_table <- read_csv("data/1_production_model/static_tables/conversion_factors.csv")
...
#| file: "scripts/1_production_model/metric_files_create.R"
#| results: hold
```
...
The final output file is `r glue::glue("egrid{params$eGRID_year}_data.xlsx")` if running annual version and `r glue::glue("egrid{params$eGRID_year}_data_monthly.xlsx")` if running monthly version.
...
if(params$temporal_res == "annual"){
  source("scripts/1_production_model/final_formatting_annual.R")
} else if(params$temporal_res == "monthly") { 
  source("scripts/1_production_model/final_formatting_monthly.R")}
...
if(params$temporal_res == "annual"){
  source("scripts/1_production_model/final_formatting_metric_annual.R")
} else if(params$temporal_res == "monthly") { 
  source("scripts/1_production_model/final_formatting_metric_monthly.R")}
...
if (ggl_file_exists) { 
  if(params$temporal_res == "annual"){
  source("scripts/1_production_model/summary_tables_create.R")
  } 
}
...
if(ggl_file_exists){
  ...
  print(glue::glue("No summary tables produced for eGRID monthly version. View egrid{params$eGRID_year}_monthly.xlsx for a data summary."))
}
