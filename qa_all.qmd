...
ifelse(nrow(check_plant_gen_sum_ann) == 0, "Status: PASS - All annual generation values between plant and generation files are equal",glue::glue("Status: NEEDS CHECKED - {nrow(check_plant_gen_sum)} plants do not have the same annual plant generation values between plant and generation files"))
...
ifelse(sum(nrow(unit_years_check),nrow(plant_years_check),nrow(generator_years_check)) == 0, "Status: PASS - Excel data year is correct in all instances.", glue::glue("Status: NEEDS CHECKED - Excel data year is not correct in {sum(c(nrow(unit_years_check),nrow(plant_years_check),nrow(generator_years_check)} instances."))
