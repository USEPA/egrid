...
<!--# Check on what is being compared -->
...
knitr::kable(only_in_epa %>% 
               select(plant_id, plant_name), caption = "EPA plants not in 'epa_plants_to_delete' list")
...
unit_excel <- data_excel[["UNT23"]]
plant_excel <- data_excel[["PLNT23"]]
gen_excel <- data_excel[["GEN23"]]
```
...
years_check <-
  rbind(unit_excel["YEAR"],
        plant_excel["YEAR"],
        gen_excel["YEAR"],
        data_excel[["ST23"]]["YEAR"],
        data_excel[["BA23"]]["YEAR"],
        data_excel[["SRL23"]]["YEAR"],
        data_excel[["NRL23"]]["YEAR"],
        data_excel[["US23"]]["YEAR"]) %>%
...
ifelse(nrow(unit_file) == nrow(unit_excel),"Status: PASS - Excel and R Unit files match in number of units.",glue::glue("Status: NEEDS CHECKED - Excel and R unit files differ by {nrow(unit_file) - nrow(unit_excel)}."))
