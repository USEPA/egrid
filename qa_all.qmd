...
ifelse(nrow(check_unit_heat_input) == 0, "All units with emissions have 
       heat input values.", 
        glue::glue("{nrow(check_unit_heat_input)} units with emissions are 
                   missing heat input values."))
...
ifelse(nrow(check_unit_emissions) == 0, "All units with heat inputs 
       have emission values.", 
        glue::glue("{nrow(check_unit_emissions)} units with heat inputs 
                   are missing emission values."))
...
ifelse(nrow(check_unit_nox_so2) == 0, "All units with positive NOx values 
       ...
       glue::glue("{nrow(check_unit_emissions)} units with positive NOx values 
                   have NA or 0 SO2 values."))
...
Check if plant locations are in regions.
...
AKGD_multi <- multi_sbrgn_shp %>% filter(subregion == "AKGD")
...
ggplot2::ggplot() + 
  ...
  ggplot2::coord_sf(xlim = c(sf::st_bbox(AKGD_plants)[1] - 5, 
                    ...
                    ylim = c(sf::st_bbox(AKGD_plants)[2] - 5, 
                             sf::st_bbox(AKGD_plants)[4] + 5), 
                    expand = TRUE) 

# this may not be necessary, and we need to include the multi files
AKGD_plants <- 
  AKGD_plants %>% 
...
ggplot2::ggplot() + 
  ...
  ggplot2::coord_sf(xlim = c(sf::st_bbox(AKMS_plants)[1] - 5, 
                    ...
                    ylim = c(sf::st_bbox(AKMS_plants)[2] - 5, 
                             sf::st_bbox(AKMS_plants)[4] + 5), 
                    expand = TRUE) 
```
...
ggplot2::ggplot() + 
  ggplot2::geom_sf(data = sbrgn_shp, ggplot2::aes(fill = subregion)) +
  ggplot2::geom_sf(data = multi_sbrgn_shp, ggplot2::aes(color = "Overlap")) +
  ggplot2::geom_sf(data = AZNM_plants) +
  ggplot2::coord_sf(xlim = c(sf::st_bbox(AZNM_plants)[1] - 5, 
                             sf::st_bbox(AZNM_plants)[3] + 5), 
                    ylim = c(sf::st_bbox(AZNM_plants)[2] - 5, 
                             sf::st_bbox(AZNM_plants)[4] + 5), 
                    expand = TRUE) 
```
