...
generator_file <- 
  read_rds(glue::glue("data/1_production_model/outputs/{params$year}/generator_file.RDS"))
...
unit_file <-
  read_rds(glue::glue("data/1_production_model/outputs/{params$year}/unit_file.RDS"))
...
xwalk_botfirty <- read_csv("data/1_production_model/static_tables/xwalk_boiler_firing_type.csv") %>% select(EPA, eGRID) %>% drop_na()
...
plant_file <- 
  read_rds(glue::glue("data/1_production_model/outputs/{params$year}/plant_file.RDS"))
...
if(length(list.files("data/shapefiles") == 1)) {
  ...
  muti_subrgn_shp <- 
    sf::st_read(glue::glue("data/1_production_model/shapefiles/eGRID2022_Mutiple_Subregions.shp")) %>%
    drop_na() %>% 
    rename("subregion" = "ZipSubregi") %>% 
    sf::st_transform(crs = "EPSG:4326")
}
...
state_agg <- read_rds(glue::glue("data/1_production_model/outputs/{params$year}/state_aggregation.RDS"))
ba_agg <- read_rds(glue::glue("data/1_production_model/outputs/{params$year}/ba_aggregation.RDS"))
subrgn_agg <- read_rds(glue::glue("data/1_production_model/outputs/{params$year}/subregion_aggregation.RDS"))
nerc_agg <- read_rds(glue::glue("data/1_production_model/outputs/{params$year}/nerc_aggregation.RDS"))
us_agg <- read_rds(glue::glue("data/1_production_model/outputs/{params$year}/us_aggregation.RDS"))
