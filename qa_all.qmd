...
plant_rates <- 
  ...
  select(plant_id, plant_name, primary_fuel_type, capfac, heat_input, heat_input_oz, generation_ann, generation_oz, nox_mass, nox_oz_mass, so2_mass, co2_mass, ch4_mass,
         n2o_mass, co2e_mass, contains("ra"), contains("ro"), contains("perc_ann_gen")) %>% 
...
knitr::kable(plant_zscore %>% 
               filter(so2_output_rate_zscore > 3 | so2_output_rate_zscore < -3) %>% 
               arrange(desc(so2_output_rate_zscore)) %>% 
               select(plant_id, plant_name, primary_fuel_type, generation_ann, ch4_mass, capfac, so2_output_rate_zscore, so2_output_rate, perc_ann_gen_coal, perc_ann_gen_oil, perc_ann_gen_gas, perc_ann_gen_biomass, perc_ann_gen_other_ff))
