...
ifelse(nrow(check_unit_blanks) == 0, "Status: PASS - No units are missing both heat inputs and emission values.", 
        glue::glue("Status: FAIL - {nrow(check_unit_blanks)} units are missing both heat and emission values."))
...
ifelse(nrow(check_unit_nox_so2) == 0, "Status: PASS - All units with positive NOx values 
       ...
       glue::glue("Status: FAIL - {nrow(check_unit_emissions)} units with positive NOx values have NA or 0 SO2 values."))
...
<!-- Ones that are different are those that are nans -->
* fix - this is not actually a difference here *
...
check_plant_lat_lon <- 
  ...
  filter(!(lat_diff == 0) | !(lon_diff == 0) | is.na(lat_diff) | is.na(lon_diff))
