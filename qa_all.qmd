...
<!--# Do we need to include 861 EIA dataset too? -->
...
<!--# Should we also exclude where annual values are zero? -->
...
check_unit_heat_input <- 
  ...
  filter((noxan > 0 | noxoz > 0 | so2an > 0 | co2an > 0 | hgan > 0) &
          (is.na(htian) & is.na(htioz))) # replaced | with &
...
<!-- Revised methodology to remove NA lats and longs from showing up  -->
...
ifelse(nrow(check_plant_ozone) == 0, "No plants have ozone values greater than annual values", 
        glue::glue("{nrow(check_plant_ozone)} plants have ozone values greater than annual values."))
...
ifelse(nrow(check_plant_duplicate) == 0, "No duplicate plants.", 
        glue::glue("{nrow(check_plant_duplicate)} duplicate plants."))
...
ifelse(nrow(check_plant_neg_emissions) == 0, "No plants have negative emission values.", 
        glue::glue("{nrow(check_plant_neg_emissions)} plants have negative emission values."))
