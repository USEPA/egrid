...
download_eia_ggl(params$eGRRID_year)


## load in datasets ----

# ggl_r = Table 10: State supply and disposition data (from EIA website) compiled into a summary sheet
ggl_r <- read_xlsx(glue::glue("data/clean_data/eia_ggl/ggl_r_{data_year}.xlsx")) 
...
state_interconnect <- state_interconnect %>%
  rename(state = State,
         state_abbr = `State Postal Code`,
         interconnect = Interconnect)

nerc_interconnect <- nerc_interconnect %>%
  rename(state = State,
...
multi_interconnect_states2 <- multi_interconnect_states %>%
  select(-interconnect) %>%
  unique()
