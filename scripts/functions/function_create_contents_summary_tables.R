...
create_contents_summary_tables <- function(contents = table_data) {
  ...
  # eGRID subregion map image
  insertImage(wb, current_worksheet, file = "data/1_production_model/static_tables/formatting/egrid_subregion_map.png",
              width = 7.24, height = 5.41, startRow = end_rows[3] + 2, startCol = start_cols)
  
  # eGRID logo
  egrid_logo <- glue::glue("data/1_production_model/static_tables/formatting/eGRIDLogoYear{params$eGRID_year}.png")
  
  if (!file.exists(egrid_logo)) {
    egrid_logo <- glue::glue("data/1_production_model/static_tables/formatting/eGRIDLogoYear2023.png")
  }
  
  insertImage(wb, current_worksheet, file = egrid_logo,
