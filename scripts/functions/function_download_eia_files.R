...
download_eia_files <- function(form, year) {
  ...
  url <- dplyr::case_when(
    form == "923" ~ glue::glue("https://www.eia.gov/electricity/data/eia923/xls/f923_{year}.zip"),
    form == "860" ~ glue::glue("https://www.eia.gov/electricity/data/eia860/xls/eia860{year}.zip"),
    form == "861" ~ glue::glue("https://www.eia.gov/electricity/data/eia861/zip/f861{year}.zip")
  )
  ...
  if (!download_and_unzip(url, dest_file, new_folder)) {
    ...
    url <- dplyr::case_when( # try URLs with "archive/" in the file path if downloads failed
    ...
    if (!download_and_unzip(url, dest_file, new_folder)) {
      ...
      if(form == "861") { 
        url <- glue::glue("https://www.eia.gov/electricity/data/eia861/zip/f861{year}er.zip") # try early release URL
