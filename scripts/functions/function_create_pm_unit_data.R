...
create_pm_unit_data <- function(){
  ...
  pm_fuel_pmover_firing <-
    ...
    mutate(pm25 = emission_factors * heat_input, pm25_source = "NEI avg EF - PM, fuel type, firing type") %>% 
    ungroup() %>%
  ...
  pm_fuel_pmover <-
    ...
    # multiply individual heat inputs by emission factors to estimate pm2.5
    # define method used under source
    mutate(pm25 = emission_factors * heat_input, pm25_source = "NEI avg EF - PM, fuel type") %>%
    ungroup() %>%
    select(plant_id, unit_id, prime_mover, pm25, pm25_source)
  ...
  pm_unit_emissions_updated <-
    pm_unit_emissions %>%
    rows_patch(pm_fuel_pmover_firing, by = c("unit_id", "plant_id", "prime_mover")) %>%
