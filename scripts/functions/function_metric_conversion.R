...
metric_conversion <- function(which_file) {
  print(glue::glue('PRODUCING {toupper(which_file)} METRIC FILE'))
  ...
  convert_to_metric <- function(data_col,var_name) {
    ...
    if (is.character(data_col)) {
      # skip variables with empty characters
      return (data_col)
    } else {
  ...
  orig_data <-read_rds(glue::glue("data/outputs/{filename}.RDS")) 
