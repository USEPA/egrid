...
The National Renewable Energy Lab (NREL) produces a dataset of utilities servicing zip codes which is used in conjunction with the eGRID production model data to assign utilities and eGRID subregions to zip codes. This data is then used in the [eGRID Power Profiler Tool](https://www.epa.gov/egrid/power-profiler#/) where users can explore their electricity fuel mix and emission rates based on their zip code and utility. 

## Architecture

Starting in 2024, EPA released the methodology to develop eGRID as an RStudio project. Recently, there has been increased interest from users in understanding the methods used to create the eGRID data. To increase transparency in the eGRID production process, EPA has made the R scripts available for users to view and use. EPA used the RStudio project beginning in 2024 to produce eGRID2023.
...
The power profiler code base is structured as follows:

-   `scripts/2c_power_profiler/`: scripts to create and format the power profile dataset.

-   `scripts/functions/`: all helper functions.

-   `data/2c_power_profiler/inputs/`: input data which includes zip code utility data and the previous year's power profiler dataset.

-   `data/2c_power_profiler/outputs/`: outputs generated by this code base constituting the power profiler dataset.

-   `data/2c_power_profiler/static_tables/`: static tables used within the code base. 

The data used to create the power profiler dataset are EIA electricity data, the eGRID annual production model dataset, the previous year's power profiler dataset, and a zip code utility assignment dataset. EIA data are acquired in the same way as the production model while the zip code utility assignment dataset is produced by NREL and is publicly available.
...
To create the power profiler dataset:
1.	Confirm the eGRID production model data has been produced and is stored in `data/1_production_model/outputs/`.
2.	Run the scripts `scripts/2c_power_profiler/power_profiler_create.R` and `scripts/2c_power_profiler/power_profiler_formatting.R`. 
...
For the power profiler, the codebase outputs two datasets as .RDS files which include zip code utility and zip code subregion assignments. The two datasets are combined and formatted to produce the final power profiler dataset and are stored in `data/2c_power_profiler/outputs/{params$eGRID_year}`.

The resulting intermediate outputs include:

-   `zip_utility_subregion.RDS`

-   `zip_subregion_assignments.RDS`

The resulting final output is:

-   `ZipSubregion{params$eGRID_year}.xlsx`
...
-   `qa_annual_comparison.qmd`: Comparison of data year outputs to previous eGRID years.
